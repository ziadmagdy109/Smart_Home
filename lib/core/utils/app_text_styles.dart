import 'package:flutter/widgets.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:smart_home/core/utils/app_colors.dart';

class AppTextStyles {
  AppTextStyles._();

  static TextStyle font12weight400 = TextStyle(
    fontSize: 12.sp,
    fontWeight: FontWeight.w400,
  );

  static TextStyle font12weight500 = TextStyle(
    fontSize: 12.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle font12weight600 = TextStyle(
    fontSize: 12.sp,
    fontWeight: FontWeight.w600,
  );
  static TextStyle font14weight500 = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w500,
  );

  static TextStyle font14weight600 = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w600,
  );
  static TextStyle font14weight400 = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w400,
  );
  static TextStyle font14Green400weight = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.myGreen,
  );
  static TextStyle font16wight500weight = TextStyle(
    fontSize: 16.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.myWhite,
  );
  static TextStyle font16wight600weight = TextStyle(
    fontSize: 16.sp,
    fontWeight: FontWeight.w600,
    color: AppColors.myWhite,
  );
  static TextStyle font17wight500weight = TextStyle(
    fontSize: 17.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle font18wight500weight = TextStyle(
    fontSize: 18.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle font18wight600weight = TextStyle(
    fontSize: 18.sp,
    fontWeight: FontWeight.w600,
  );

  static TextStyle font18wight700weight = TextStyle(
    fontSize: 18.sp,
    fontWeight: FontWeight.w700,
  );

  static TextStyle font22weight700 = TextStyle(
    fontSize: 22.sp,
    fontWeight: FontWeight.w700,
  );

  static TextStyle font28weight700 = TextStyle(
    fontSize: 28.sp,
    fontWeight: FontWeight.w700,
  );
  static TextStyle font20weight700 = TextStyle(
    fontSize: 20.sp,
    fontWeight: FontWeight.w700,
  );

  static TextStyle font24weight600 = TextStyle(
    fontSize: 24.sp,
    fontWeight: FontWeight.w600,
  );

  static TextStyle font32bold = TextStyle(
    fontSize: 32.sp,
    fontWeight: FontWeight.bold,
  );
}
